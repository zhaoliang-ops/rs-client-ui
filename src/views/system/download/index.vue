...
<script>
import { listLocalimage, getLocalimage,getDownLoadLocalimageList, delLocalimage, addLocalimage, updateLocalimage,editP2PLocalimage } from "@/api/system/localimage";
  ...
  methods: {
    ...
    getProgress() {
     ...
     for(var i=0;i<downList.length;i++){
       if(downList[i].promotionId==1 && downList[i].wrsRow == 0){
          if(downList[i].diskId == 100){
            this.queryParams.imageId = downList[i].imageId
            this.queryParams.promotionId = 2
            this.queryParams.downloadTime = this.getDownloadTime(downList[i].dataSize)
     ...
     if(this.timer == '' || !downNum){
        console.log("无任务清除定时器")
       this.clearTimer()
     }
    },
    /** 查询本地资源管理列表 */
    getList() {
      this.loading = true;
      // this.clearTimer()
      getDownLoadLocalimageList(this.queryParams).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认取消下载该编号为"' + imageIds + '"的遥感数据？').then(function() {
